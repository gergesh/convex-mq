...
A typed message queue component for Convex, enabling external services to consume messages reactively via Convex subscriptions.
...
src/client/             Client library (compiled to dist/)
  index.ts              MessageQueue<V> class — typed via Convex validators, api() generates exports
  client.ts             consume() (reactive) and consumePolling() helpers for external consumers

convex/                 Host app for development/testing (uses the component)
  convex.config.ts      Installs component as emailQueue and taskQueue
  emailQueue.ts         Uses emailQueue.api(query, mutation) — one-line setup
  taskQueue.ts          Uses taskQueue.api(query, mutation) — one-line setup

test/                   Tests (all vitest)
  messageQueue.test.ts  Unit tests via convex-test (including claimId race condition tests)
  consume.integration.test.ts   Integration tests against live deployment
  concurrent.integration.test.ts  Concurrent consumer + queue isolation tests

rust-consumer/          Example Rust consumer using the convex crate
examples/               Example JS consumer scripts
```
